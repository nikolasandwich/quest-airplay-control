...
async function run(){
 ...
 const document={getElementById(name){return elements[name]||(elements[name]={textContent:'',style:{}});}};
 ...
 fetch:(_url,options)=>++calls===1?Promise.resolve({ok:true,json:async()=>({active:true,stage:0,baseline:true,session:'test',ageMs:0,status:'ready'})}):new Promise((_,reject)=>{options.signal?.addEventListener('abort',()=>{aborted++;reject(Error('aborted'));});})};
 ...
 assert.match(elements.connection?.textContent||'',/暂不可用/,'stalled HTTP must not keep a live/synchronized status');
 ...
 console.log('PASS: request deadlines, independent stale status, retries, and no pursuit target');
}
